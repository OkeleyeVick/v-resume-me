...
const Navigation = () => {
	...
	return (
		<div className="flex items-center justify-between h-full border-r-border_clr border-r">
			<div className="h-full">
				<div className="py-[0.3rem] px-[0.2rem]">
					<Link
						to="/"
						className="relative flex flex-col items-center border-2 border-main p-1 gap-y-1 py-4 rounded-tl-[10px] rounded-br-[10px]">
					...
					{navLinks.map(({ icon_name, linkname, component, size }, index) => {
						return (
							<React.Fragment key={index}>
								<button
									type="button"
									className={`relative overflow-hidden h-[55px] md:h-[60px]
									${
										buttonActive === index ? "text-main" : "hover:bg-opacity-80 text-gray-300"
